...
function get_bot (i, DarKBoZorG)
	function bot_info (i, DarKBoZorG)
		redis:set("botBOT-IDid",DarKBoZorG.id_)
		if DarKBoZorG.first_name_ then
			redis:set("botBOT-IDfname",DarKBoZorG.first_name_)
		end
		if DarKBoZorG.last_name_ then
			redis:set("botBOT-IDlanme",DarKBoZorG.last_name_)
		end
		redis:set("botBOT-IDnum",DarKBoZorG.phone_number_)
		return DarKBoZorG.id_
	end
...
function is_DarKBoZorG(msg)
    local var = false
	local hash = 'botBOT-IDadmin'
	local user = msg.sender_user_id_
    local DarKBoZorG = redis:sismember(hash, user)
	if DarKBoZorG then
		var = true
	end
	return var
...
function process_join(i, DarKBoZorG)
	if DarKBoZorG.code_ == 429 then
		local message = tostring(DarKBoZorG.message_)
...
function process_link(i, DarKBoZorG)
	if (DarKBoZorG.is_group_ or DarKBoZorG.is_supergroup_channel_) then
		redis:srem("botBOT-IDwaitelinks", i.link)
		redis:sadd("botBOT-IDgoodlinks", i.link)
	elseif DarKBoZorG.code_ == 429 then
		local message = tostring(DarKBoZorG.message_)
		local Time = message:match('%d+')
			...
			if is_DarKBoZorG(msg) then
				...
				elseif text:match("^(لیست) (.*)$") then
					...
					if matches == "مخاطبین" then
						...
						function (I, DarKBoZorG)
							local count = DarKBoZorG.total_count_
							local text = "مخاطبین : \n"
							for i =0 , tonumber(count) - 1 do
								local user = DarKBoZorG.users_[i]
					...
					elseif matches == "مسدود" then
						DarKBoZorG = "botBOT-IDblockedusers"
					elseif matches == "شخصی" then
						DarKBoZorG = "botBOT-IDusers"
					elseif matches == "گروه" then
						DarKBoZorG = "botBOT-IDgroups"
					elseif matches == "سوپرگروه" then
						DarKBoZorG = "botBOT-IDsupergroups"
					elseif matches == "لینک" then
						DarKBoZorG = "botBOT-IDsavedlinks"
					elseif matches == "مدیر" then
						DarKBoZorG = "botBOT-IDadmin"
					else
						return true
					end
					local list =  redis:smembers(DarKBoZorG)
					local text = tostring(matches).." : \n"
					for i, v in pairs(list) do
						text = tostring(text) .. tostring(i) .. "-  " .. tostring(v).."\n"
					end
					writefile(tostring(DarKBoZorG)..".txt", text)
					tdcli_function ({
						ID = "SendMessage",
						...
						input_message_content_ = {ID = "InputMessageDocument",
							document_ = {ID = "InputFileLocal",
							path_ = tostring(DarKBoZorG)..".txt"},
						caption_ = "لیست "..tostring(matches).." های تبلیغ گر شماره BOT-ID"}
					}, dl_cb, nil)
					return io.popen("rm -rf "..tostring(DarKBoZorG)..".txt"):read("*all")
				...
				elseif text:match("^(تازه سازی)$")then
					...
					tdcli_function({
						...
						redis:set("botBOT-IDcontacts", DarKBoZorG.total_count_)
					end, nil)
					for i, v in pairs(list) do
							for a, b in pairs(v) do 
								tdcli_function ({
									...
									if  DarKBoZorG.ID == "Error" then rem(i.id) 
									end
								end, {id=b})
							end
					end
				...
				elseif tostring(msg.chat_id_):match("^-") then
					...
					elseif text:match("^(افزودن همه مخاطبین)$") then
						tdcli_function({
							...
							local users, count = redis:smembers("botBOT-IDusers"), DarKBoZorG.total_count_
							for n=0, tonumber(count) - 1 do
								tdcli_function ({
									ID = "AddChatMember",
									chat_id_ = i.chat_id,
									user_id_ = DarKBoZorG.users_[n].id_,
									forward_limit_ = 50
								},  dl_cb, nil)
							end
							for n=1, #users do
